...
import { FULL_USER_DATA, FULL_USER_DATA_INPUT_TYPES, FULL_USER_DATA_INPUT_LABELS } from "../../utils/consts"
...
function Contacts(props: ContactsProps): React.ReactElement {
  ...
  function addNewContact() {
    ...
    dispatch(addContact(name as string|undefined, email as string|undefined, avatar as string|undefined, phones as [string]|undefined, quote as string|undefined, index as string));
    setNewContactData({
      name: '',
      email: '',
      avatar: '',
    ...
    <section className="Contacts">
      ...
      <ul className={props.presentationList ? "Contacts__list" : "Contacts__cells"}>
        {contactsState.map((contact, index) => (
          <li key={`contact-${index}`} className={`Contacts__list-item ${props.presentationList && "Contacts__list-item_list"}`}>
        ...
        <button onClick={togglePopup} className="Contacts__add-button">+</button>
        <section className={`Contacts__add-popup ${isPopupOpened && "Contacts__add-popup_opened"}`}>
        <button onClick={togglePopup} className="Contacts__close-popup">+</button>
        <form className="Contacts__add-form">
        {FULL_USER_DATA.map((field, index) => (
          <fieldset key={`field-${field}-${index}`} className={`Contacts__form-fieldset ${field === 'avatar' && "Contacts__form-fieldset_avatar"}`}>
            <input onChange={(e) => setCurrentContactData(e.target.value, field)} className="Contacts__form-input" type={FULL_USER_DATA_INPUT_TYPES[index]} placeholder={`Введите ${FULL_USER_DATA_INPUT_LABELS[index].toLowerCase()}`} name={`${field}-input`} id={`${field}-input`} />
            <label className="Contacts__form-label" htmlFor={`${field}-input`}>Введите {FULL_USER_DATA_INPUT_LABELS[index].toLowerCase()}</label>
          </fieldset>
        ))}
        <button type="button" onClick={addNewContact} className="Contacts__form-submit">Добавить</button>
        </form>
        </section>
      </ul>
    </section>
